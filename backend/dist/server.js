...
const looksLikePlaceholder = (value) => {
    ...
    return (normalized.includes('seu_') ||
...
const localBannerDirectory = path_1.default.resolve(process.cwd(), 'public', 'login-banner');
...
const wiseBaseUrl = normalizeEnv(process.env.WISE_BASE_URL) ||
...
const allowVercelPreviewOrigins = normalizeEnv(process.env.ALLOW_VERCEL_PREVIEW_ORIGINS).toLowerCase() === 'true';
...
const isOriginAllowed = (origin) => {
    if (!origin)
        return true; // server-to-server ou same-origin sem header
    ...
    try {
        ...
        if (!isLocalhostOrigin && parsed.protocol !== 'https:')
            return false;
        if (allowVercelPreviewOrigins && vercelDomainRegex.test(parsed.hostname))
            return true;
    }
    catch {
        return false;
...
const validPaymentMethods = new Set(['credit_card', 'pix', 'boleto']);
const paymentIntents = new Map();
const paymentByOrderAndKey = new Map();
const PAYMENT_INTENT_TTL_MS = 30 * 60 * 1000;
const MAX_CONFIRM_ATTEMPTS = 5;
const rateLimitBuckets = new Map();
let wiseTokenCache = null;
if (!supabaseUrl || !supabaseAuthKey) {
    console.warn('[SECURITY] SUPABASE_URL/SUPABASE_AUTH_KEY nao configurados corretamente no backend. ' +
...
const fetchWithTimeout = async (input, init, timeoutMs = requestTimeoutMs) => {
...
app.use('/static/login-banner', express_1.default.static(localBannerDirectory, {
    fallthrough: true,
    index: false,
    etag: true,
    maxAge: '1h',
...
const normalizePaymentMethod = (value) => {
    ...
    if (!validPaymentMethods.has(normalized))
        return null;
    return normalized;
};
...
const normalizeText = (value, minLength = 1, maxLength = 200) => {
    if (typeof value !== 'string')
        return null;
...
const normalizeCardMonth = (value) => {
    if (typeof value !== 'string')
        return null;
    const digits = value.replace(/\D/g, '');
...
const parseDatabaseAmount = (value) => {
    const amount = typeof value === 'number' ? value : Number(value);
    return Number.isFinite(amount) ? Math.round(amount * 100) / 100 : 0;
};
const amountMatchesOrder = (requested, orderTotal) => {
    return Math.abs(requested - parseDatabaseAmount(orderTotal)) <= 0.01;
};
const buildOrderKey = (userId, orderId, idempotencyKey) => `${userId}:${orderId}:${idempotencyKey}`;
const buildSupabaseHeaders = (serviceKey, json = false) => {
    return {
        apikey: serviceKey,
...
const paymentStatusToOrderStatus = (paymentMethod, status) => {
    if (status === 'cancelled' || status === 'declined' || status === 'refunded')
        return 'cancelled';
...
const webhookStatusToInternal = (status) => {
    if (typeof status !== 'string')
        return null;
    const normalized = status.trim().toLowerCase();
    if (['approved', 'paid', 'confirmed'].includes(normalized))
        return 'approved';
    if (['processing', 'in_analysis'].includes(normalized))
        return 'processing';
...
const getSupabaseOrderById = async (config, orderId) => {
...
const getSupabaseOrderByPaymentId = async (config, paymentId) => {
...
const getSupabaseOrderItems = async (config, orderId) => {
...
const getSupabaseProductsByIds = async (config, productIds) => {
...
const calculateOrderTotalFromCatalog = async (config, orderId) => {
...
const updateSupabaseOrder = async (config, orderId, updates) => {
    const response = await fetchWithTimeout(`${config.url}/rest/v1/orders?id=eq.${encodeURIComponent(orderId)}`, {
        method: 'PATCH',
...
const safeTimingEqual = (provided, expected) => {
...
app.post('/api/payment/webhook', webhookRateLimit, express_1.default.raw({ type: 'application/json', limit: '100kb' }), async (req, res) => {
    try {
        ...
        const expectedSignature = crypto_1.default.createHmac('sha256', webhookSecret).update(rawBody).digest('hex');
...
const requireBlackcatConfig = (res) => {
...
const mapBlackcatStatus = (status) => {
    const normalized = typeof status === 'string' ? status.toLowerCase() : '';
    if (normalized === 'aprovado' || normalized === 'approved')
        return 'aprovado';
    if (normalized === 'pendente' || normalized === 'pending')
        return 'pendente';
    if (normalized === 'rejeitado' || normalized === 'declined' || normalized === 'failed')
        return 'rejeitado';
    return 'erro';
};
const requestBlackcatApi = async (config, endpoint, payload) => {
    const controller = new AbortController();
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const message = (errorPayload && typeof errorPayload.mensagem === 'string' && errorPayload.mensagem) ||
...
const requireWiseConfigured = (res) => {
...
const getWiseAccessToken = async () => {
    ...
    if (!response.ok) {
        ...
        throw new Error(payload.error_description || payload.error || `Falha ao autenticar na Wise (HTTP ${response.status})`);
    }
...
const requestWiseApi = async (endpoint, init = {}, headers = {}) => {
...
const validateBlackcatAddress = async (req, res) => {
    try {
        ...
        const response = await requestBlackcatApi(config, '/endereco/validar', {
            logradouro: street,
            numero: number,
...
const processBlackcatPayment = async (req, res) => {
    try {
        const config = requireBlackcatConfig(res);
        if (!config)
            return;
        ...
        const blackcatPayload = {
            ...
            reference_id: `${Date.now()}-${crypto_1.default.randomUUID().slice(0, 8)}`,
            cliente: {
        ...
        const response = await requestBlackcatApi(config, '/pagamento/processar', blackcatPayload);
...
const refundBlackcatPayment = async (req, res) => {
    try {
        const config = requireBlackcatConfig(res);
        if (!config)
            return;
        ...
        const response = await requestBlackcatApi(config, '/pagamento/reembolsar', {
...
const getBlackcatPaymentStatus = async (req, res) => {
    try {
        const config = requireBlackcatConfig(res);
        if (!config)
            return;
        ...
        const response = await requestBlackcatApi(config, '/pagamento/consultar', {
            transaction_id: transactionId,
        });
...
const getWiseProfileId = async (requestedProfileId) => {
    ...
    const profiles = await requestWiseApi('/v2/profiles', { method: 'GET' });
...
const createWiseQuote = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
            return;
        ...
        const quote = await requestWiseApi(`/v3/profiles/${profileId}/quotes`, { method: 'POST', body: JSON.stringify(payload) });
...
const getWiseQuote = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        if (!quoteId)
            return res.status(400).json({ error: 'quoteId invalido' });
        const profileId = await getWiseProfileId(req.query.profileId);
        const quote = await requestWiseApi(`/v3/profiles/${profileId}/quotes/${encodeURIComponent(quoteId)}`, { method: 'GET' });
        return res.status(200).json({
...
const getWiseRecipientRequirements = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        if (!quoteId)
            return res.status(400).json({ error: 'quoteId invalido' });
        const requirements = await requestWiseApi(`/v1/quotes/${encodeURIComponent(quoteId)}/account-requirements`, { method: 'GET' }, { 'Accept-Minor-Version': '1' });
        return res.status(200).json(requirements.map((item) => ({
            key: String(item.key || ''),
            type: item.type || 'text',
...
const createWiseRecipient = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        const recipient = await requestWiseApi('/v1/accounts', {
            method: 'POST',
            body: JSON.stringify({
                currency,
                type,
...
const getWiseTransferRequirements = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        const requirements = await requestWiseApi('/v1/transfer-requirements', {
            method: 'POST',
            body: JSON.stringify(payload),
        });
        return res.status(200).json(requirements.map((item) => ({
...
const createWiseTransfer = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        const profileId = await getWiseProfileId(req.body?.profileId);
        const details = normalizeObject(req.body?.details) || {};
        const transfer = await requestWiseApi(`/v1/transfers`, {
            method: 'POST',
            body: JSON.stringify({
                targetAccount: Number(recipientId),
                quoteUuid: quoteId,
...
const fundWiseTransfer = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        const profileId = await getWiseProfileId(req.body?.profileId);
        const payment = await requestWiseApi(`/v3/profiles/${profileId}/transfers/${encodeURIComponent(transferId)}/payments`, {
            method: 'POST',
...
const getWiseTransferStatus = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
        if (!requireWiseConfigured(res))
        ...
        const transferId = normalizeText(req.params.transferId, 1, 80);
        if (!transferId)
            return res.status(400).json({ error: 'transferId invalido' });
        const transfer = await requestWiseApi(`/v1/transfers/${encodeURIComponent(transferId)}`, {
            method: 'GET',
        });
        return res.status(200).json({
...
const handleWiseWebhookEvent = async (req, res) => {
    try {
        if (!req.authUser?.id)
            return res.status(401).json({ error: 'Usuario nao autenticado' });
...
const initiateCheckoutPayment = async (req, res) => {
    try {
        ...
        const now = Date.now();
        const paymentId = `pay_${crypto_1.default.randomUUID().replace(/-/g, '')}`;
        const initialStatus = paymentMethod === 'credit_card' ? 'processing' : 'pending';
        const intent = {
            paymentId,
            orderId,
            userId,
...
const confirmCheckoutPayment = async (req, res) => {
    try {
...
const getCheckoutPaymentStatus = async (req, res) => {
    try {
        ...
        const paymentStatus = intent?.status || (order?.status === 'processing' ? 'confirmed' : order?.status) || 'pending';
...
const refundCheckoutPayment = async (req, res) => {
    try {
        const paymentId = normalizePaymentId(req.body?.paymentId);
        const amount = normalizeAmount(req.body?.amount);
...
app.get('/api/public/login-banner', (req, res) => {
    const localBannerFile = path_1.default.basename((process.env.LOGIN_BANNER_FILE || '').trim());
    const fallbackBannerUrl = (process.env.LOGIN_BANNER_IMAGE_URL || '').trim();
    if (localBannerFile) {
        const localFilePath = path_1.default.join(localBannerDirectory, localBannerFile);
...
app.get('/api/payout/wise/quotes/:quoteId/account-requirements', wisePayoutRateLimit, authenticate, getWiseRecipientRequirements);
